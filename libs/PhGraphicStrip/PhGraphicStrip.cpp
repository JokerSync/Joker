...
void PhGraphicStrip::draw(int x, int y, int width, int height)
{
    ...
    if(height > 0)
    {
		long pixelPerFrame = _settings->horizontalSpeed();

		if(getTextFont()->getBoldness() != _settings->textBoldness())
			getTextFont()->setBoldness(_settings->textBoldness());
		if(getTextFont()->getFontFile() != _settings->textFontFile())
			getTextFont()->setFontFile(_settings->textFontFile());
...
#warning TODO simplify this because appHeight = y
        int appHeight = 1;
		if(_settings->stripHeight())
